...
const COLORS = ['\x1b[32m', '\x1b[36m', '\x1b[29m', '\x1b[33m', '\x1b[37m', '\x1b[38m', '\x1b[39m', '\x1b[40m'];
...
const start = () => {
    ...
    for (let f of fileList) {
        ...
        Object.keys(specWeights).forEach(spec => {
            // console.log('is f ' + f + ' ends with ' + spec);
            if (f.endsWith(spec)) {
    ...
    /*for (let f of fileList) {
        const specWeight = specWeights[f] ? specWeights[f].weight : DAFAULT_WEIGHT;
        map.set(f, specWeight);
    }*/
    ...
    console.log(weigths);
    const commands = weigths.map((w, i) => ({ color: COLORS[i], tests: `'${w.list.join(',')}'` }));
                ...
                if (titleRegexp.test(data)) {
                    ...
                    try {
                       // console.log(data, 'DATA');
                        ...
                        if (test[0] === 'pass') {
                            // log(`✔ ${test[1].title}`);
                            console.log(`\x1b[32m✔ \x1b[0m${test[1].title} (${test[1].duration}ms)`);
                        }
                        if (test[0] === 'fail') {
    ...
    Promise.all(children).then(resultMaps => {
        ...
        Object.keys(specWeights).forEach(spec => {
            specWeights[spec].weight = Math.floor((specWeights[spec].time / totalDuration) * totalWeight);
        });
    ...
    /*let promise2 = new Promise((resolve, reject) => {
        let result = '';
        let isResult = false;
        const p2 = spawn('npm.cmd', [
            'run',
            'cy:run',
            "'--spec'",
            "'cypress\\integration\\create-job-panel\\discard-modl.integration.ts,cypress\\integration\\navigation\\navigation.integration.ts'"
        ]);
        ...
        p2.stderr.on('data', data => {
            console.log('\x1b[31m', `[CY-PARALLEL 2]\n${data}`);
        });
    ...
    // "npm run cy:run -- --spec 'cypressintegrationcreate-job-panel\technician-selector.integration.ts,cypressintegrationspec3working-hours.integration.ts"

    /*exec('npm run cy:run -- --spec cypress\\integration\\views\\weekly-view.integration.ts', function(
        error,
        stdout,
        stderr
    ) {
    ...
    // exec('node_modules/.bin/cypress run --spec cypress\\integration\\views\\weekly-view.integration.ts');
};
